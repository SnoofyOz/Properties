...
const Portal = () => {
    ...
    const fetchPortals = () => {
        ...
        setLoading(true);
        fetch('http://localhost:8080/v1/api/admin/portals', {
            headers: {
    ...
    const handleDelete = (id) => {
        if (window.confirm("Bạn có chắc chắn muốn xóa user này không?")) {
            const token = localStorage.getItem('token');
            fetch(`http://localhost:8080/v1/api/admin/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
    ...
    const handleAddUser = () => {
        ...
        const token = localStorage.getItem('token');
        fetch('http://localhost:8080/v1/api/admin', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json',
    ...
    const handleUpdateUser = () => {
        ...
        const token = localStorage.getItem('token');
        fetch(`http://localhost:8080/v1/api/admin/${editingUser.id}`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json',
